...
C
     ...
     1      INTEGER*4 (I-N)
        CHARACTER*6 KWORD
        CHARACTER*22 BUFFER,OFILE,OFILE1,OFILE2,OFILE3,OFILE4,
     1               OFILE5,OFILE6
C
...
C
        PARAMETER (NGT=1650)
        COMMON/DCSTAB/ECS,TCS1,TCS2,TH(NGT),THLAB(NGT),XT(NGT),NTAB,
     1                DCST(NGT),DCSTLAB(NGT),SPOL(NGT),ERROR(NGT)
...
C   from Ch. Kittel, 'Introduction to Solid State Physics'. 
C   5th ed. (John Wiley and Sons, New York, 1976).
C   The value -1.0D-8 indicates that the experimental value
C   for the element was not available.
C
        DIMENSION DNNEL(103)
...
C
        CALL CPU_TIME(START)
C
C   Input data. Default model.
C
        IELEC =-1        ! electron
        IZ    = 82       ! lead-208
        NELEC = 0        ! just nucleus
        MNUCL = 2        ! sum of Gaussians charge distribution
        MELEC = 3        ! DF electron density
        MUFFIN= 0        ! free atom
        RMUF  = 0        ! free atom
        ...
        ISOT  = 0        ! isotop indicator number  (=1 only for Ca-48)
        UNC  = 0         ! different forms of Vabs
C
  100   CONTINUE
        ...
        ELSE IF(KWORD.EQ.'ISOT') THEN
            READ(BUFFER,*) ISOT
        ELSE IF(KWORD.EQ.'IFR') THEN
            READ(BUFFER,*) IFR
            IF(MABS.EQ.0) IFR=0
        ELSE IF(KWORD.EQ.'UNC') THEN
            READ(BUFFER,*) UNC
...
C
 ...
 1004   FORMAT(1X,'#',/1X,'#   Energy',9X,'ECS',9X,'TCS1',9X,'TCS2',
  ...
  300   CONTINUE
        ...
        EVLAB=EV 
        EVLABGEV=EVLAB*1.0D-9
        WRITE(6,*) 'Etot_lab (eV)=',EVLAB
C
...
C
        ...
        IF(IZ.EQ.20.AND.ISOT.EQ.1) THEN
            TARGMASS=0.9314940954D0*47.9525D0  
        ENDIF
        
        SINVTARG=TARGMASS**2+2.0D0*TARGMASS*EVLABGEV+BEAMMASS**2
        ECM=(SINVTARG+BEAMMASS**2-TARGMASS**2)/(2.0D0*SQRT(SINVTARG))
        ECMKIN=ECM-BEAMMASS
        EV=ECMKIN*1.0D9
        
        WRITE(6,*) 'Ekin_CM (eV) =',EV
        WRITE(6,*) 'Etot_CM (eV) =',EV+BEAMMASS*1.0D9
        IF(EV.LT.10.0D0) STOP 'The kinetic energy is too small'
C
        IF (MABS.EQ.0) THEN
            PRINT '(/,A,/)',' Absorptive potential is off'        
            OFILE4='NoVabs_'
            OFILE5=''
            OFILE6=''
            GO TO 400
        ELSE
            PRINT '(/,A,/)',' Absorptive potential is on'        
            OFILE4=''
        ENDIF
C
C   Record the value of the Compton slope
C   used for this particular nucleus 
C      
        CALL COMPSLOPE(BP,BCOMP,DBCOMP,IZ,ISOT)
        WRITE(BUFFER,'(1P,I0)') NINT(BP)
        OFILE5='BP'//TRIM(ADJUSTL(BUFFER))//'_'
C
C   Record the model used for the parametrization
C   of the absorptive potential
C
        IF (UNC.EQ.0) THEN
            OFILE6='UNC0_'
        ELSE IF (UNC.EQ.1) THEN
            OFILE6='UNC1_'
        ELSE IF (UNC.EQ.2) THEN
            OFILE6='UNC2_'
        ELSE IF (UNC.EQ.3) THEN
            OFILE6='UNC3_'
        ELSE
            WRITE(6,*) 'Change the value for UNC --> 0...3'
            STOP        
        ENDIF
...
C
        WRITE(BUFFER,'(1P,E12.5,I1)') EVLAB
        OFILE=BUFFER(2:2)//'p'//BUFFER(4:6)//'e'//BUFFER(11:12)
        WRITE(BUFFER,'(1P,I1.1)') MNUCL        
        OFILE1='_m'//BUFFER(1:1)
        WRITE(BUFFER,'(1P,I0)') IZ        
        OFILE2='Z'//TRIM(ADJUSTL(BUFFER))//'_'
        WRITE(BUFFER,'(1P,I0)') ISOT
        OFILE3='IS'//TRIM(ADJUSTL(BUFFER))//'_'
        OPEN(8,FILE='dcs_'//TRIM(ADJUSTL(OFILE4))//
     -  TRIM(ADJUSTL(OFILE5))//TRIM(ADJUSTL(OFILE2))//
     -  TRIM(ADJUSTL(OFILE3))//TRIM(ADJUSTL(OFILE6))//
     -  OFILE(1:8)//OFILE1(1:3)//'.dat')
C
        IF(MCPOL.NE.0) THEN
            IF(EV.GT.1.0D4) THEN
                WRITE(6,*) 'WARNING: For E>10 keV the correlation-polari',
     1                     'zation correction is switched off.'
                MCPOLC=0
                VPOLBC=0.0D0
            ELSE
                MCPOLC=MCPOL
        ...
        ENDIF
        IF(MABS.NE.0) THEN
            IF(EV.GT.1.0D10) THEN
                WRITE(6,*) 'WARNING: For E>10 GeV, the absorption correc',
     1                      'tion is switched off.'
        ...
        ENDIF
C
C   Partial-wave analysis is implemented by calling the main
C   ELSEPA subroutine.
C
        CALL ELSEPA(IELEC,EV,IZ,NELEC,MNUCL,MELEC,MUFFIN,RMUF,
     1              MEXCH,MCPOLC,VPOLA,VPOLBC,MABSC,VABSA,VABSD,
     2              IHEF,8,0.0D0,ISOT,IFR,UNC)
C
        ERRM=0.0D0
        DO I=1,NTAB
...
C
        ...
        WRITE(6,*) 'Calculation time=',FINISH-START,' s'
      END
